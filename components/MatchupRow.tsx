    ...
    <div className="flex items-center justify-between bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
      ...
      <div className="flex items-center gap-4">
        <div className="text-right">
          <div className="text-white font-semibold">{game.away_team}</div>
          {game.away_record && (
            <div className="text-xs text-gray-400">{game.away_record}</div>
          )}
        </div>
        ...
        <div>
          <div className="text-white font-semibold">{game.home_team}</div>
          {game.home_record && (
            <div className="text-xs text-gray-400">{game.home_record}</div>
          )}
        </div>
      </div>
      ...
      <div className="text-center text-xs text-gray-300">
        {game.game_time && (
          <div className="text-sm text-white mb-1">{game.game_time}</div>
        )}
        <div>
          {game.away_pitcher} <span className="text-gray-500">vs</span>{" "}
          {game.home_pitcher}
        </div>
      </div>
